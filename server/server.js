...
io.on("connection", function (socket) {
  ...
  socket.on("somebodyJoin", function (data) {
    socket.join("vision-group-chat-1", () => {
      let rooms = Object.keys(socket.rooms)
      console.log(rooms)
      io.to("vision-group-chat-1").emit("getSomebodyJoin", data) // broadcast to everyone in the room
    })
  })

  socket.on("groupChat", function (data) {
    console.log(data)
    GroupChatModel.create(data, (err, doc) => {
      io.emit("groupChatMessage", Object.assign({}, doc._doc))
    })
  })
